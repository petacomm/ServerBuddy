...
HISTORY_PATH = Path.home() / ".petacomm" / "history.json"
MAX_HISTORY = 6
...
def clean_response(text):
    text = re.sub(r'PETACOMM_RUN:.*$', '', text, flags=re.MULTILINE)
    text = re.sub(r'PETACOMM_CONFIRM:.*$', '', text, flags=re.MULTILINE)
    text = re.sub(r'PETACOMM_PARALLEL:.*$', '', text, flags=re.MULTILINE)
...
2. For safe READ-ONLY commands, put ONLY this on the very last line:
   PETACOMM_RUN: <full shell command>
...
3. For MULTIPLE INDEPENDENT modifying tasks that can run in PARALLEL (e.g. install nginx AND mysql AND redis):
   ...
   PETACOMM_PARALLEL: [{{"name":"nginx","cmd":"apt install -y nginx"}},{{"name":"mysql","cmd":"apt install -y mysql-server"}}]

4. For a SINGLE modifying operation:
   Explain what will happen, list risks, then end with EXACTLY this on the last line:
   PETACOMM_CONFIRM: <exact command> | DANGER: low|medium|high
...
7. If a previous command failed, remember it and suggest fixing the root cause."""
    ...
    parallel_match = re.search(r'PETACOMM_PARALLEL:\s*(\[.+?\])\s*$', turn1_text, re.MULTILINE | re.DOTALL)
